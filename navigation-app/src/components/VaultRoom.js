...
const VaultRoom = () => {
  ...
  useEffect(() => {
    const unlocked = localStorage.getItem('vaultRoomUnlocked');
    if (unlocked === 'true') {
      setIsUnlocked(true);
      navigate('/room/보급실/next');
    }
  }, [navigate]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (password === '4776') {
      localStorage.setItem('vaultRoomUnlocked', 'true');
      setIsUnlocked(true);
      navigate('/room/보급실/next');
    } else {
  ...
  return (
    <div className="room-container" style={{ backgroundImage: "url('/images/보급실.png')" }}>
      <h2>보급실</h2>
      <img src="/images/002.png" alt="Hint" className="hint-image" />
